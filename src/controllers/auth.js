...
import { getRandInteger, sendMail } from '../middleware/helperFunctions.js';
        ...
        bcrypt.hash(password, 10, (bcryptErr, hash) => {
          if (bcryptErr) {
            ...
            pool.connect((insertError, insertClient, insertDone) => {
              ...
              const randomCode = getRandInteger('0', '7000');
...
export const generateVCode = (req, res) => {
  pool.connect((err, client, done) => {
  ...
  pool.connect((err, client, done) => {
    ...
    client.query(checkQuery, checkValue, (error, result) => {
      ...
      if (result.rows < '1') {
        ...
        const verificationCode = getRandInteger('0', '7000');
...
export const verifyPasswordChangeToken = (req, res) => {
...
export const changeForgotPassword = (req, res) => {
  pool.connect((err, client, done) => {
    const { email, password } = req.body;
    const checkQuery = 'SELECT * FROM users WHERE email = $1';
    const checkValue = [email];
